...
To do that, it calls a service that tries to map each nif to a category. In case of unsuccess, it uses a list of known invoice emitters, if there's no match, it tries to associate based on keywords for each
category. If no match found too, it doesn't select any category.
